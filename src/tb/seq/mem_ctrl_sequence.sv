class mem_ctrl_sequence extends uvm_sequence #(mem_ctrl_seq_item);
    ...
    task send_op(input op_codes_e op_val, output mem_ctrl_seq_item item);
        ...
        start_item(item);
        if (!item.randomize() with {op == op_val;})
            `uvm_fatal(get_name(), "Randomization failed")
        finish_item(item);
    endtask

    task send_read_data(input logic [16:0] addr, output mem_ctrl_seq_item item);
        item = mem_ctrl_seq_item::type_id::create("item");
        start_item(item);
        if (!item.randomize() with {op == OP_READ_DATA; item.addr == addr;})
            `uvm_fatal(get_name(), "Randomization failed")
        finish_item(item);
    endtask
    ...
    task body();
        ...
        if (item.error) begin
            ...
            `uvm_error(get_name(), $sformatf("[FAIL] READ_ID got=0x%06X expected=0x%06X", item.rdata, DEVICE_ID))
            fail_cnt++;
        end else begin
        ...
        if (item.error) begin
            ...
            `uvm_info(get_name(), $sformatf("[PASS] READ_STATUS rdata=0x%02X", item.rdata[7:0]), UVM_LOW)
        end
        ...
        if (item.error) begin
            ...
            `uvm_error(get_name(), $sformatf("[FAIL] READ_DATA got=0x%02X expected=0x%02X", item.rdata[7:0], rw_data))
            fail_cnt++;
        end else begin
            `uvm_info(get_name(), $sformatf("[PASS] READ_DATA rdata=0x%02X", item.rdata[7:0]), UVM_LOW)
        end
