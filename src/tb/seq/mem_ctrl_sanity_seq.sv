class mem_ctrl_sanity_seq extends uvm_sequence #(mem_ctrl_seq_item);
    ...
    task body();
        ...
        if (item.error) begin
            ...
            `uvm_error(get_name(), $sformatf("[SANITY FAIL] READ_ID: got=0x%06X expected=0x%06X",
        ...
        if (item.error) begin
            ...
            `uvm_error(get_name(), $sformatf("[SANITY FAIL] READ_DATA: got=0x%02X expected=0x%02X",
