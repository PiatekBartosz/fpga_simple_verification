// Microchip IC model from https://www.microchip.com/en-us/product/at24cm02
...
// C. Tomlinson 02/17/2020 AT24CM02 1.8V to 5.5V 400kHz max 10ms tWC
...
//AT24CM02, 1024 pages at 256 bytes/page, 18 address bits
`ifdef AT24CM02
...
//AT24CM01, 512 pages at 256 bytes/page, 17 address bits
`ifdef AT24CM01
...
// AT24C512C, 512 pages at 128 bytes/page, 16 address bits 
`ifdef AT24C512
  parameter MEM_SIZE = 65536;	 
  parameter ADDR_SIZE = 16;
  parameter WORD_ADDR_SIZE = 7;
...
// AT24C256C, 512 pages at 64 bytes/page, 15 address bits
`ifdef AT24C256C
...
// AT24C128C, 256 pages at 64 bytes/page, 14 address bits
`ifdef AT24C128C
...
// AT24CM02, 1024 pages at 256 bytes/page, 18 address bits 
parameter MEM_SIZE_2MEG_BIT = 262144;	 
parameter ADDR_SIZE_2M_BIT = 18;
parameter WORD_ADDR_SIZE_2M_BIT = 9;
...
parameter TIMEOUT_FOR_WRITE = 10_000_000;	// 10 ms for AT24CM02 tWC cjt 02/17/2020
...
always @(posedge SCL) 
  if (ld_addr_reg_MSB_byte == 1) begin
    ...
    `ifdef AT24CM02 
      addr_reg[ADDR_SIZE-3: 8] <= S_Byte_Shft_Reg;
    `endif
    `ifdef AT24CM01 
      addr_reg[ADDR_SIZE-2: 8] <= S_Byte_Shft_Reg;
    `endif
    `ifdef AT24C1024 
      addr_reg[ADDR_SIZE-2: 8] <= S_Byte_Shft_Reg; 	// Fixed 24C1024
    `endif
    `ifdef AT24C512 
...
`ifdef AT24CM02
  Device_Address = S_Byte_Shft_Reg[7: 3];
  if (Device_Address == SLAVE_ADDRESS) 
  begin if (VERBOSE) $display ("Valid device address for AT24CM02");  
  Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
    $display ("Valid device address not detected for AT24CM02");
  end
`endif
...
`ifdef AT24CM01
  ...
  if (Device_Address == SLAVE_ADDRESS) 
  begin if (VERBOSE) $display ("Valid device address for AT24CM01");  
  Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
    $display ("Valid device address not detected for AT24CM01");
  end
`endif
...
`ifdef AT24C512C
  ...
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C512C");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
    $display ("Valid device address not detected for AT24C512C");
  end
`endif
...
`ifdef AT24C256C
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C256C");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
    $display ("Valid device address not detected for AT24C256C");
  end
`endif
...
`ifdef AT24C128C
  Device_Address = S_Byte_Shft_Reg[7: 1];
  if (Device_Address == SLAVE_ADDRESS) 
    begin if (VERBOSE) $display ("Valid device address for AT24C128C");  
      Valid_Address_flag = 1;
  end else if (VERBOSE) begin
    $display ($realtime);
    $display ("Valid device address not detected for AT24C128C");
  end
`endif
...
task Get_Lower_Device_Address_Bits_and_ACK_to_Master;  
  ...
  `ifdef AT24CM02  
     addr_reg[ADDR_SIZE-2] = S_Byte_Shft_Reg[2:1];  //Grab P1 & P0 
  `endif
  `ifdef AT24CM01  
