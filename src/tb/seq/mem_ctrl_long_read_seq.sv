class mem_ctrl_long_read_seq extends uvm_sequence #(mem_ctrl_seq_item);
    ...
    task body();
        ...
        `uvm_info(get_name(), $sformatf(
                  "=== Long read seq start: %0d bytes at 0x%05X ===", NUM_BYTES, BASE_ADDR),
                  UVM_NONE)
        ...
        for (int i = 0; i < NUM_BYTES; i++) begin
            ...
            if (item.error) begin
                `uvm_error(get_name(), $sformatf(
                           "[FAIL] WRITE[%0d] addr=0x%05X", i, cur_addr))
                fail_cnt++;
            end else `uvm_info(get_name(), $sformatf(
                               "[PASS] WRITE[%0d] addr=0x%05X data=0x%02X",
                               i, cur_addr, expected[i]), UVM_LOW)
        end
        ...
        for (int i = 0; i < NUM_BYTES; i++) begin
            ...
            if (item.error) begin
                `uvm_error(get_name(), $sformatf(
                           "[FAIL] READ[%0d] addr=0x%05X returned error", i, cur_addr))
                fail_cnt++;
            end else if (item.rdata[7:0] !== expected[i]) begin
                `uvm_error(get_name(), $sformatf(
                           "[FAIL] READ[%0d] addr=0x%05X got=0x%02X expected=0x%02X",
                           i, cur_addr, item.rdata[7:0], expected[i]))
                fail_cnt++;
            end else `uvm_info(get_name(), $sformatf(
                               "[PASS] READ[%0d] addr=0x%05X data=0x%02X",
                               i, cur_addr, item.rdata[7:0]), UVM_LOW)
        end
